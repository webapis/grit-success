...
export default function ProductContainer({ data, slugObj }) {
    ...
    return <>
        ...
        <Grid container gap={0} sx={{ display: 'flex', justifyContent: { xs: 'center', md: 'start' }, padding: 0, margin: 0 }}>
            {data.map((m, i) => { return <Grid sx={{ padding: 0, margin: 0 }} item key={i} xs={6} sm={4} md={3} lg={2}> <Image slugObj={slugObj} handleSelect={handleSelect} matchingCategories={[...slugObj.keywords, ...slugObj.positives.flat(), ...slugObj.words]} obj={m} subcat={''} /></Grid> })}</Grid></>
}