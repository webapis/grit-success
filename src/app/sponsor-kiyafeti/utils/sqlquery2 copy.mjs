...
for (let category of mergedCategories) {
 ...
 if(exceptions){
    ...
    const candiateData = data.filter((f) => !searchObject(f, genders)).filter((f) => negative ? !searchObject(f, [...otherNegatives,...negative]) : !searchObject(f,otherNegatives)).filter(f => {
