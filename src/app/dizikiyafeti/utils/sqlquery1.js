...
pool.connect().then(() => {
  ...
  pool.request().query('select * from View_DiziKiyafeti', async (err, result) => {
    ...
    debugger
    const groubByDizi = groupBy(objects, 'TVSeriesTitle')
    ...
    debugger
    fs.writeFileSync(`${process.cwd()}/src/app/dizikiyafeti/page-data/dizikiyafeti.json`, JSON.stringify(objects), { encoding: 'utf8' })
