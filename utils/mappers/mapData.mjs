...
export default async function mapData({ sourceFolder, destinationFolder, fileName }) {
    ...
    try {
        ...
        const mappedPrice = flattenedData.filter(f => f.price).map(m => {
            const price = mapPrice(m.price, m)
            return { ...m, price, priceFormatted: formatPriceAsTurkishLira(price), priceAsString: m.price }
        })
